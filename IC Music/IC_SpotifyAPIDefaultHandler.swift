...
class IC_SpotifyAPIDefaultHandler: NSObject, ObservableObject {
    
    
    static let shared = IC_SpotifyAPIDefaultHandler()
    
    
    // MARK: - API
    
    func fetchAccessToken(responseCode: String?) -> AnyPublisher<IC_SpotifyAccessToken, Error> {
        ...
        return dataTaskPublisher
            ...
            .map(\.data)
            .tryMap { data -> IC_SpotifyAccessToken in
                let decoder = JSONDecoder()
                do {
                    return try
                    decoder.decode(IC_SpotifyAccessToken.self, from: data)
                } catch {
